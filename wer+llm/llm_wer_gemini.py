...
# Resolve log file path and define a tee for stdout/stderr mirroring
...
class LLMTranscriptProcessor:
    ...
    def _extract_json_object(self, text: str) -> str:
        """Extract the first top-level JSON object from text (best-effort)."""
        if not text:
            return ""
        cleaned = text.strip()
    ...
    def _parse_llm_json_result(self, result_text: str) -> Dict[str, Any]:
        try:
    ...
    def _finalize_metrics(self, result: Dict[str, Any], reference: str) -> Dict[str, Any]:
        if not isinstance(result, dict):
            result = {}
        subs = result.get("substitution_pairs") or []
        dels = result.get("deletion_words") or []
        ins = result.get("insertion_words") or []
        result["substitutions"] = result.get("substitutions") or (len(subs) if isinstance(subs, list) else 0)
        result["deletions"] = result.get("deletions") or (len(dels) if isinstance(dels, list) else 0)
        result["insertions"] = result.get("insertions") or (len(ins) if isinstance(ins, list) else 0)
...
Analyze these transcripts and return only the JSON response:"""
    ...
    def calculate_conversational_wer_with_llm(self, reference: str, hypothesis: str) -> Dict[str, Any]:
        ...
        try:
            # Avoid str.format on templates containing JSON braces; do safe token replacement
            ...
            # Robust parsing and optional retry
            result = self._parse_llm_json_result(result_text)
            if not result:
...
def main():
    ...
    with open(OUTPUT_LOG_FILE, "w", encoding="utf-8") as log_file:
        ...
        sys.stdout = tee
        sys.stderr = tee
        # Also add a file handler for logging to ensure logs are captured
        ...
        try:
            ...
            output_file = "global_wer_gemini_llm.json"
            ...
            # Detach file handler and restore std streams
