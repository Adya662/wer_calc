...
# ------------------------------------------------------------------------------
# Core WER Processing Function (Fixed to use Gemini API)
# ------------------------------------------------------------------------------
def process_transcripts(folder: str, ref_name: str, hyp_name: str):
    ...
    # Updated LLM prompt - specifically for bot utterances only
    prompt = (
        ...
        "You are processing ONLY bot/assistant utterances from voice conversations. The input contains filtered utterances from either 'assistant' or 'Agent' speakers only.\n\n"
        "## Task Overview\n"
        ...
        "  ]\n"
        "}\n\n"
        "IMPORTANT: The input contains ONLY bot/assistant utterances. Do not include any user/customer words or artifacts. Process only the 'content' field from each utterance in the input JSON array.\n\n"
        ...
        for attempt in range(1, max_retries + 1):
            try:
                ...
                if attempt < max_retries:
                    wait = 2 ** attempt
                    print(f"Warning: Gemini API error, retrying in {wait}s (attempt {attempt}/{max_retries}): {e}")
                    time.sleep(wait)
                else:
    ...
    with ThreadPoolExecutor() as executor:
        ...
        future_hyp = executor.submit(call_gemini_llm, hyp_text)
        resp_ref = future_ref.result()
        resp_hyp = future_hyp.result()
    
    # Save and validate responses (handle potential JSON wrapper from Gemini)
    ...
    # Enhanced transcript cleaning - remove any remaining user/customer artifacts
    def clean_transcript(text):
        import re
        # Remove speaker prefixes and user artifacts more aggressively
        text = re.sub(r"\b(user|customer|client|caller|user:|customer:|client:|caller:)\s*", "", text, flags=re.IGNORECASE)
        # Remove common user interjections that might leak through
        text = re.sub(r"\b(hello|hi|yes|yeah|ok|okay|hmm|um|uh)\s+", "", text, flags=re.IGNORECASE)
        # Remove standalone user acknowledgments
        text = re.sub(r"\b(yes\s+ma'am|yes\s+sir|no\s+ma'am|no\s+sir)\b", "", text, flags=re.IGNORECASE)
...
def calculate_global_wer():
    ...
    # Since these are already from filtered JSON canonical_transcript fields,
    # they contain ONLY bot utterances - no additional filtering needed
    global_ref_text = " ".join(all_canonical_ref_texts)
    global_hyp_text = " ".join(all_canonical_hyp_texts)
    
    print(f"🌍 Calculating Global WER from bot-only canonical texts...")
    print(f"   📊 Total calls processed: {len(all_canonical_ref_texts)}")
    ...
    print(f"   🎯 Reference word count: {len(global_ref_text.split())}")
    print(f"   🎯 Hypothesis word count: {len(global_hyp_text.split())}")
    ...
    try:
        ...
        print(f"   ➕ Global insertions: {global_wer_output.insertions}")
        print(f"   ➖ Global deletions: {global_wer_output.deletions}")
        print(f"   🔄 Global substitutions: {global_wer_output.substitutions}")
        ...
        global_wer_details = {
            ...
            "total_ref_words": len(global_ref_text.split()),
            "total_hyp_words": len(global_hyp_text.split()),
            "individual_call_wers": wer_values,
            "average_individual_wer": sum(wer_values) / len(wer_values) if wer_values else 0.0,
            "note": "Global WER calculated from concatenated bot-only canonical transcripts"
        }
